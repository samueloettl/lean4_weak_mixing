...
theorem onAverageIndependent_measurableSet_of_piSystem {C : Set (Set α)}
    ...
    · have (A : Set α) : Tendsto (birkhoffAverage ℝ (preimage f)
      ...
      have hAnTendsto' (m : ℕ) : Tendsto (birkhoffAverage ℝ (Prod.map id (preimage f))
        ...
        have pairwise_disjoint (x : ℕ) :
          ...
          fun _ _ _ _ hij _ h1 h2 ↦
            (dis hij (subset_inter_iff.mp h1|>.1) (subset_inter_iff.mp h2|>.1))
        have measurable (x : ℕ) :
            ∀ i ∈ Finset.range m , MeasurableSet (sn i ∩ (preimage f)^[x] t) :=
          fun i _ ↦ MeasurableSet.inter (hsnmeas i)
            (MeasurableSet.congr (hMP.iterate x|>.measurable hBmeas)
              (congrFun preimage_iterate_eq t))
...
theorem measurePreserving_of_onAverageIndependent [IsProbabilityMeasure μ] (hM : Measurable f)
    (h : ∀ B, MeasurableSet B → OnAverageIndependent univ B f μ) : MeasurePreserving f μ μ := by
  ...
  have h' := h (f ⁻¹' S) (hM hS)
  specialize h S hS
  simp_all only [Set.univ_inter, probReal_univ, one_mul]
...
theorem of_onAverageIndependent [IsProbabilityMeasure μ] (hM : Measurable f)
    (h : ∀ A, MeasurableSet A → ∀ B, MeasurableSet B → OnAverageIndependent A B f μ) :
    Ergodic f μ := by
  refine ⟨measurePreserving_of_onAverageIndependent hM <|h univ .univ, ⟨fun B hB hBi ↦ ?_⟩⟩
  ...
  simp only [Set.compl_inter_self, measure_empty, ENNReal.toReal_zero, zero_eq_mul] at h
...
theorem onAverageIndependent [IsProbabilityMeasure μ]
  ...
  have := Filter.Tendsto.inner (𝕜:=ℝ)
    (tendsto_const_nhds (x:=indicatorConstLp 2 hA (measure_ne_top μ _) (1:ℝ)) (f:=atTop (α:=ℕ)))
    (tendsto_birkhoffAverage_const ℝ (indicatorConstLp 2 hB (measure_ne_top μ _) (1:ℝ)) h)
  have average_lin := fun t ↦ map_birkhoffAverage ℝ ℝ
    (innerₛₗ ℝ ((indicatorConstLp 2 hA (measure_ne_top μ _) 1):Lp ℝ 2 μ))
  ...
  rw [integral_indicatorConstLp, funext average_lin, smul_eq_mul _ 1, mul_one] at this
  simp only [birkhoffAverage, birkhoffSum, ← indicatorConstLp_univ,
    L2.inner_indicatorConstLp_indicatorConstLp] at this
  simpa [Lp.compMeasurePreserving_iterate, Lp.indicatorConstLp_compMeasurePreserving,
    preimage_iterate_eq, L2.real_inner_indicatorConstLp_one_indicatorConstLp_one,
    real_inner_eq_re_inner] using this
